...
import type { NormalizationStats, ObliqueTreeNode, ProductVector } from './decisionTree';
...
function calculateBranchStatistics(
  ...
  featureNames.forEach((attrName, idx) => {
    const values = products
      .map((p) => {
        const raw = p.rawValues?.[idx];
        if (raw !== undefined && !isNaN(raw)) {
          return raw;
        }
        const val = p.values[idx];
        if (normalization && normalization.mins[idx] !== undefined && normalization.maxs[idx] !== undefined) {
          const range = normalization.maxs[idx] - normalization.mins[idx];
          if (range === 0) return normalization.mins[idx];
          return normalization.mins[idx] + val * range;
        }
        return val;
      })
...
export function analyzeSplit(
  ...
  const normalization = node.normalization;
  ...
  const leftStats = calculateBranchStatistics(leftProducts, featureNames, attributeProfiles, normalization);
  const rightStats = calculateBranchStatistics(rightProducts, featureNames, attributeProfiles, normalization);
